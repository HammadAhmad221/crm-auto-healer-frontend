...
const LoadList = () => {
  ...
  useEffect(() => {
    const fetchLoadsAndInvoices = async () => {
      try {
        const loadsResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}api/loads`, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        });

        const invoicesResponse = await axios.get(`${import.meta.env.VITE_BACKEND_URL}api/invoices`, {
          headers: {
            Authorization: localStorage.getItem('token'),
          },
        });
        ...
        // Combine loads with their respective invoices
